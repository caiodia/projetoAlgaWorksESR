...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	 ...
	 * 
	 * ProblemType problemType = ProblemType.DADOS_INVALIDOS; String detail =
	 * "Um ou mais campos estão inválidos. Faça o preenchimento correto e tente novamente."
	 * ;
	 * 
	 * // Uma instancia de BindingResult armazena as violações de constraints de //
	 * validação, dentro desse cara temos acesso as propriedades que foram //
	 * violadas. BindingResult bindingResult = ex.getBindingResult();
	 * 
	 * List<Problem.Field> problemFields = bindingResult.getFieldErrors().stream()
	 * // aqui foi usado a API stream do java e gerou um stream de Problem.Field
	 * .map(fieldError -> Problem.Field.builder().name(fieldError.getField())
	 * .userMessage(fieldError.getDefaultMessage()).build()) // no segundo passo
	 * geramos um List de Problem.Field, resultado que será // atribuido ao objeto
	 * problemFields. .collect(Collectors.toList());
	 * 
	 * Problem problem = createProblemBuilder(status, problemType,
	 * detail).userMessage(detail).fields(problemFields) .build();
	 * 
	 * return handleExceptionInternal(ex, problem, headers, status, request); }
	 */
